...
class BilibiliVideoPipeline(chat_pipeline.IChatPipeline):
    ...
    def execute(self, qq_group_number: str, qq: str, msg: str) -> str:
        ...
        resp = None
        if match_obj:
            resp = bili.get_video_info(qq_group_number, qq, aid=int(match_obj.group(1)))
        else:
            match_obj = re.match(r'^(?:https?://(?:www\.bilibili\.com/video|b23\.tv)/)?bv([0-9A-Za-z]{10})$', msg, re.I)
            if match_obj:
                resp = bili.get_video_info(qq_group_number, qq, bid=match_obj.group(1))
            else:
                match_obj = re.match(r'^https?://b23\.tv/[0-9A-Za-z]{7}$', msg, re.I)
                if match_obj:
                    resp = bili.get_video_info(qq_group_number, qq, short_url=match_obj.group(0))
        if resp is not None:
