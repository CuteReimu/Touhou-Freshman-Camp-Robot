...
class Bilibili:
    ...
    def get_live_status(self, qq_group_num: int) -> None:
        ...
        if live_status_resp['data']['live_status'] == 0:
            mirai_bot.send_group_message(qq_group_num, [plain('直播间状态：未开播')])
        else:
    ...
    def start_live(self, qq_group_num: int, qq: int) -> None:
        bili_jct = self.cookies.get('bili_jct')
        if bili_jct is None:
            mirai_bot.send_group_message(qq_group_num, [plain('B站登录过期')])
            return
        rid = config.bilibili['room_id']
        ...
        mirai_bot.send_temp_message(qq_group_num, qq, [plain('RTMP推流地址：{0}\n秘钥：{1}'.format(rtmp_addr, rtmp_code))])

    def stop_live(self, qq_group_num: int) -> None:
        bili_jct = self.cookies.get('bili_jct')
        if bili_jct is None:
            return mirai_bot.send_group_message(qq_group_num, [plain('B站登录过期')])
        rid = config.bilibili['room_id']
        ...
        if stop_live_resp['data']['change'] == 0:
            mirai_bot.send_group_message(qq_group_num, [plain('直播间本来就是关闭的')])
        else:
            mirai_bot.send_group_message(qq_group_num, [plain('直播间已关闭')])

    def change_live_title(self, qq_group_num: int, title: str) -> None:
        bili_jct = self.cookies.get('bili_jct')
        if bili_jct is None:
            return mirai_bot.send_group_message(qq_group_num, [plain('B站登录过期')])
        rid = config.bilibili['room_id']
        ...
        if change_live_title_resp['code'] != 0:
            ...
            mirai_bot.send_group_message(qq_group_num, [plain('修改直播间标题失败，请联系管理员')])
        else:
            mirai_bot.send_group_message(qq_group_num, [plain('直播间标题已修改为：' + title)])
